 ...
 * zrl
 */
class shop_module extends api_front implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {	
		...
		$store_name 	 	= trim($this->requestData('store_name', ''));
		$contact_phone		= trim($this->requestData('contact_phone', ''));
		$store_address		= trim($this->requestData('store_address', ''));
		...
		if (!in_array('shop', $return_shipping_range)) {
			return new ecjia_error('return_shipping_range_error', '当前申请不支持到店退货返还方式！');
		}
