...
<div class="row-fluid batch" >
	<div class="choose_list span12">
		<form class="f_l" name="searchdateForm" action='{url path="refund/admin_payment_refund/init"}' method="post">
			...
			<button class="btn select-button" type="button">筛选{$filter.refund_status}</button>
		</form>
		<form method="post" action='{url path="refund/admin_payment_refund/init"}{if $filter.start_date}&start_date={$filter.start_date}{/if}{if $filter.end_date}&end_date={$filter.end_date}{/if}' name="searchForm">
		...
		<table class="table table-striped table-hide-edit" data-rowlink="a">
			...
			<tr>
				<td class="hide-edit-area">{$list.order_sn}
					<div class="edit-list">
						<a href='{url path="refund/admin_payment_refund/payment_refund_info" args="id={$list.id}"}' class="data-pjax" title="查看">查看</a>
					</div>
				</td>
